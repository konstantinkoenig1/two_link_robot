...
model_dir = "./models_and_logs/DDPG_larger_first_layer"
os.makedirs(model_dir, exist_ok=True)

# Create environment without rendering
env = FixedBaseRobotEnv(findings_log_path="models_and_logs/DDPG_larger_first_layer/findings.txt") # not render_mode = "human" 

# Configure custom network architecture
policy_kwargs = dict(activation_fn=th.nn.ReLU,
                     net_arch=dict(pi=[800, 300], qf=[800, 300]))

# Create the model
model = DDPG('MlpPolicy', env, policy_kwargs=policy_kwargs, verbose=1, tensorboard_log=model_dir)
# model = DDPG.load(os.path.join(model_dir, "ppo_fixed_base_robot"), env=env, tensorboard_log=model_dir)
